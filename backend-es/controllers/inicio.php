<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    class inicio extends MX_Controller {
        ...
        public function __construct(){
            ...
            $this->fv = 'inicio'; // form validation variable
    ...
    <link href="plugins/gallery/pretty-photo/prettyPhoto.css" type="text/css" rel="stylesheet" />
        ...
        public function nuevo(){
            ...
            if ($this->form_validation->run($this->fv) == FALSE ){
                ...
                $id = $this->minicio->insertar($pData);
        ...
        public function editar($id){
            ...
            if (isset($_POST['titulo'])){
                if ($this->form_validation->run($this->fv) == FALSE ){
                        ...
                        for($i=0; $i< $totalImagenes; $i++){
                            ...
                            if ($tipo == 'foto'){
                                ...
                                if ( ! $this->upload->do_upload($imagenes[$i]))
                                {
                                    ...
                                    $this->data['contenido'] = $this->load->view('inicio/editar_view',$data,true);
                                }
                                else
                                {
                                    $dataFile = $this->upload->data($imagenes[$i]);
                    ...
                    $this->minicio->actualizar($pData,$id);
                    redirect($this->router->class);
                }
            }
            // Obteniendo la informacion
            $info = $this->minicio->getById($id);
        ...
        public function eliminar_foto()
        {
            ...
            if($campo !="0"){$foto      = $this->minicio->getCampo('foto_'.$campo,$id);
                $thumbnail = $this->minicio->getCampo('thumbnail_'.$campo,$id);// Borrando los archivos fisicamente
